...
public class PublishGamesController {
    ...
    public void initialize() {
        ...
        // 异步加载游戏数据
        loadGamesWithStatusOffline();
    }

    /**
     * 加载状态为"下架"的游戏数据
     */
    private void loadGamesWithStatusOffline() {
        new Thread(() -> {
            try {
                // 调用API获取状态为"下架"的厂商游戏数据
                ...
                requestData.put("status", "下架"); // 默认查询下架状态的游戏
    ...
    private StackPane createGameCard(VendorMainController.Game game) {
        ...
        // 添加点击事件 - 点击卡片进行上架申请
        card.setOnMouseClicked(event -> handleShelfApplication(game));
     ...
     * 处理上架申请
     */
    private void handleShelfApplication(VendorMainController.Game game) {
        // 显示确认对话框
        boolean confirmed = ControllerUtils.showConfirmationAlert(
            "上架申请确认",
            "确认要对游戏 '" + game.getName() + "' 进行上架申请吗？",
            "上架申请后需要等待管理员审核"
        );
        ...
        if (confirmed) {
            ...
            new Thread(() -> {
                try {
                    // 调用API进行上架申请
                    String endpoint = "/vendors/create-game-application";
                    Map<String, Object> requestData = new HashMap<>();
                    ...
                    javafx.application.Platform.runLater(() -> {
                        if (result != null && result.contains("成功")) {
                            ControllerUtils.showInfoAlert("上架申请提交成功，请等待管理员审核");
                            // 重新加载游戏列表
                            loadGamesWithStatusOffline();
                        } else {
                            ControllerUtils.showErrorAlert("上架申请提交失败: " + result);
                        }
                    });
                } catch (Exception e) {
                    javafx.application.Platform.runLater(() -> {
                        ControllerUtils.showErrorAlert("上架申请提交失败: " + e.getMessage());
                    });
                }
            }).start();
        }
